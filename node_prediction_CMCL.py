...
def load_train_metric(args):
    if args.dataset == "arxiv":
        df_train_metric = pd.read_csv("/data/nidhi/ogbn_arxiv/ogbn_arxiv_col_indices.csv")
    
    if args.dataset == "cora":
        df_train_metric = pd.read_csv("/data/nidhi/cora/cora_col_indices_v2_80_10_10.csv")
...
def main():
    ...
    for run in range(args.runs):
        ...
        proba_train = np.amax(logits_softmax)
        ...
        logits = out[split_idx['val']].detach().cpu().numpy()
        logits_softmax = softmax(logits, axis=1)
        proba_val = np.amax(logits_softmax)
        ...
        logits_softmax = softmax(logits, axis=1)
        proba_test = np.amax(logits_softmax)
        np.savetxt(f'/competence_based_multiview_CL/acl_23_multiview_ccl_gnn/node_prediction/val_{args.dataset}_{args.feat}_{prioritizing_approach}.out', proba_test, delimiter=',') 
        '''
